...
export default function HyperOverlay({
  ...
  }, [active, contactOpen, pages, contactLabel]);
    ...
    const tick = (time) => {
      ...
      if (frame % 8 === 0) {
        if (velocityRef.current)
          velocityRef.current.textContent = Math.abs(smoothVelocity).toFixed(2);
        if (coordRef.current)
          coordRef.current.textContent = String(
            Math.round((activeNow + contactBoost) * 1000),
          ).padStart(6, "0");
        if (fpsRef.current)
          fpsRef.current.textContent = String(Math.round(1000 / deltaTime));
      }
